...
The multiview uses a fixed grid of three rows and four columns, with up to 12 created channels per page. Previous/next controls switch pages, closing the outgoing page's players and opening ready channels on the selected page. Offline, disabled, and error channels keep their tiles; unused grid positions stay blank without placeholder channels. The channel set refreshes automatically. Drag a tile by its title bar onto another tile or a previous/next page button to reorder it, or click the title bar to choose a destination position using the keyboard or touch. The size-reset button remains independently clickable. Title bars show the channel name and status dot without status text or a fullscreen entry button. Order is saved in this browser for the current Gateway origin, not shared between users or included in project snapshots. New channels are appended and deleted channels are removed from the arrangement. Legacy `/view/CHANNEL_ID` links open the same multiview. Each channel receives the lowest available positive number, producing compact URLs such as `http://192.168.15.5:8080/embed/1`. Existing channels keep their number, but a number becomes available for reuse after its channel is deleted. The embed route renders only the muted video surface: no header, status overlay, native controls, border, or opaque page background. Previously issued UUID embed URLs remain valid aliases for their existing channels.
