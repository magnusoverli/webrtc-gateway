...
Gateway starts a supervised `srt-live-transmit` listener with a 20 ms cabled-LAN latency default, a 4 MiB receive buffer, three-second peer-idle detection, and one-second connection timeout. Increase the per-channel latency for lossy or long-distance links. FFmpeg's SRT `latency` URL value is expressed in microseconds, so `20000` matches the Gateway's 20 ms setting. Gateway classifies each SRT message before routing it into MediaMTX. Raw MPEG-TS and RTP/MP2T are remuxed by FFmpeg to normalize transport headers, codec metadata, and PES framing. FFprobe first tries a 200 ms startup snapshot with a 500 ms probe budget. Incomplete early discovery falls back to a fresh snapshot at the original one-second mark, within one shared four-second discovery budget (16 MiB per snapshot, 32 MiB total retained input). Both attempts require complete program maps and resolved tracks; all retained bytes are replayed in order. Incomplete or oversized final discovery fails visibly and retries instead of guessing track codecs. Elementary RTP is forwarded directly over loopback UDP. Internal publishers exist only while a sender is active, and the listener recovers automatically after disconnects and Gateway restarts.
...
### Recovery After SDI Switching

Players replace their WHEP session when the output generation changes, including when an entire source restart happens between status polls. The first three playback retries wait approximately 250 ms each (with jitter), then back off through one, two, four, and eight seconds. A newly ready output generation resets that backoff. Old reader deletion is coordinated before replacement with a bounded wait.

A serial browser media-progress check runs every 500 ms while visible. After video has started, two seconds without further decoded frames triggers recovery even if audio and the WebRTC connection continue. Where decoded-frame counters are unavailable it monitors received bytes instead. Audio-only output is monitored through received bytes; empty negotiated video placeholders are ignored. New media/decoders get a twelve-second startup grace for their first packets/frame, and shorter gaps can recover without reconnecting. Hidden pages pause the checks and reset their evidence on return. This is a recovery heuristic for live media; very low-frame-rate feeds or unusually long keyframe intervals may need different tuning.

For the shortest hardware-encoder interruption, keep the switcher SDI output format fixed and use continuous encoder output on SDI loss if supported. Ask the encoder for an immediate IDR with repeated codec headers when signal returns, and use a short GOP (for example 0.5 seconds). Progressive 8-bit 4:2:0 H264 without B-frames avoids video conversion when validated; AAC still requires audio conversion. Baseline profiles have the fastest conservative classification path. SRT latency settings do not remove SDI relock time or the wait for usable video. Gateway does not reuse a previous connection's codec decision after a reconnect.

Gateway logs `SRT MPEG-TS discovery complete` and `SRT compatibility inspection complete` with per-channel `durationMs` to identify startup costs. Measure switching with the same browser page open, through the first valid displayed post-switch frames; an API `outputReady` flag or a connected ICE session alone is not proof of resumed video.
...
- Decoder and encoder parallelism is bounded, and CRF 23 output uses a per-channel maximum video bitrate (5 Mbps by default), bounded by the resolution-based ceiling, with a half-second VBV buffer.
...
Resolution-based video ceilings are `2 Mb/s` through 480p, `6 Mb/s` through 720p, `16 Mb/s` through 1080p, `24 Mb/s` through 1440p, and `40 Mb/s` above 1440p. **Edit channel → Compatibility video limit (Mbps)** sets an additional per-channel ceiling from 0.1 to 40 Mbps; the lower ceiling applies. New channels, existing channels upgraded from an older release, and older project files default to **5 Mbps**. The setting is saved in project exports and exposed as integer `compatibilityVideoMaxKbps` in the channel API (5000 = 5 Mbps); omitting it on update preserves the current value, and zero resets it to the default. It applies only when video is encoded for compatibility, not direct passthrough or video copied during audio-only conversion. Converted audio adds approximately 128 kbps, and transport overhead is additional. These are maximum encoder rates rather than constant target rates, so simple content remains smaller and short bursts can exceed the ceiling. The VBV buffer is half the selected maximum rate. Changing the effective ceiling restarts that channel's video compatibility worker and can briefly interrupt its output.

A managed relay signals the compatibility manager after accepting valid media, invalidates stale MediaMTX runtime status, and checks startup every 100 ms for up to three seconds; the one-second periodic check remains as a fallback. H264 Baseline or missing-profile inputs first try low-analysis current-stream metadata (256 KiB probe size, 200 ms analysis, at most 1.5 seconds). Incomplete or contradictory results fall back to two seconds of frame sampling within the same four-second budget; known Main/High profiles go directly to structural inspection. Incomplete MediaMTX metadata receives a 500 ms grace period before independent inspection. Every source generation is validated afresh, including codec, dimensions, pixel format, B-frame, and interlace checks.
...
For browser recovery regression coverage, run `node scripts/recovery-browser.mjs` from `web` with Playwright and Chrome available (the same `PLAYWRIGHT_MODULE` and `CHROME_CHANNEL` overrides apply). It uses a local Vite server on port 5175 and real browser media peers to verify an already-open embed survives a short gap, replaces its session on a new output generation, recovers video stalls while audio continues, and deletes every retired reader. It reports generation-to-first-presented-frame and stall-to-first-presented-frame times; these measure the local browser fixture, not hardware SDI/SRT recovery.
