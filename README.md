...
Multiview tiles have no native player controls and remain muted. Their audio meters analyze decoded browser audio independently of video muting, showing RMS levels from -60 to 0 dBFS, a one-second sample-peak hold, and a near-full-scale indicator at -0.5 dBFS. These are sampled level meters, not true-peak or LUFS measurements. Select **Enable meters** if the browser requires a user gesture to start audio analysis. Mono and stereo are labelled M and L/R when the receiver reports channel count; unknown counts show the first two decoded slots as 1/2. Up to eight reported decoded channels are shown. Meters reflect browser output, including any upstream or browser downmix, not necessarily every source audio channel. Missing audio, suspended analysis, and unavailable metering are distinct from measured silence. Analysis pauses when hidden and releases tile resources on page changes.
