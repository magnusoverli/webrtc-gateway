...
The multiview uses a fixed grid of three rows and four columns, with up to 12 created channels per page. Previous/next controls switch pages, closing the outgoing page's players and opening ready channels on the selected page. Offline, disabled, and error channels keep their tiles; unused grid positions stay blank without placeholder channels. The channel set refreshes automatically. Drag a tile's handle onto another tile or a previous/next page button to reorder it, or click the handle to choose a destination position using the keyboard or touch. Order is saved in this browser for the current Gateway origin, not shared between users or included in project snapshots. New channels are appended and deleted channels are removed from the arrangement. Legacy `/view/CHANNEL_ID` links open the same multiview. Each channel receives the lowest available positive number, producing compact URLs such as `http://192.168.15.5:8080/embed/1`. Existing channels keep their number, but a number becomes available for reuse after its channel is deleted. The embed route renders only the muted video surface: no header, status overlay, native controls, border, or opaque page background. Previously issued UUID embed URLs remain valid aliases for their existing channels.

While dragging, a level, actual-size translucent tile follows the pointer using one captured video frame (or the tile's text status when no frame is available). Its reserved grid position is invisible. A neighbor is displaced only when its unanimated slot overlaps strictly more than half the preview's full rectangular area, regardless of handle position. Other tiles animate into the proposed positions without restarting their players. The proposal stays in place through gaps and partial overlaps; dropping with the preview center inside any occupied tile slot (including its invisible reserved slot) saves it. Escape, a cancelled pointer, or dropping with the preview center outside those slots restores the arrangement. Page buttons instead use the pointer position: hover highlights the destination without opening its players before drop. Reduced-motion preferences disable layout animation. The preview never creates an extra WebRTC session or video decoder.
